...
# Updated with the verified File IDs you provided
FILES = {
...
def load_and_prep_data():
    ...
    # --- CRITICAL FIX: Robust Date Formatting ---
    # Ensure years are integers (e.g., "2009", not "2009.0") to match notebook IDs
    years = pd.to_numeric(df["year"], errors='coerce').fillna(0).astype(int).astype(str)
    years = years.replace("0", "NA")
    
    # Reconstruct movie_id exactly as it was in the training notebook
    df["movie_id"] = df["title"].astype(str).str.strip() + " (" + years + ")"
    ...
    # 5. Fast Matrix Factorization using SVD (Calculated on the fly)
...
try:
    ...
    # --- Display Results (Fixed: No matplotlib dependency) ---
    st.divider()
    if recs:
        st.subheader("Top Recommendations")
        df_recs = pd.DataFrame(recs, columns=["Movie Title", "Similarity Score"])
        
        # Using Streamlit's native column config for formatting and bars
        st.dataframe(
            df_recs,
            column_config={
                "Movie Title": st.column_config.TextColumn("Movie Title"),
                "Similarity Score": st.column_config.ProgressColumn(
                    "Score",
                    format="%.4f",
                    min_value=0,
                    max_value=max(df_recs["Similarity Score"].max(), 1.0)
                )
            },
